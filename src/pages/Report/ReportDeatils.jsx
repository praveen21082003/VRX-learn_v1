...
import { getViewUrl } from "@services/Issues.service";
// utils
...
function ReportDetails() {
  ...
  const { getIssue, updateIssue, loading, updating, fetchError, issue } =
    useReportIssues();
  ...
  const {
    ...
    getViewUrl,
    {
      404: "No attachment found for this issue",
      403: "You don't have permission to view this attachment",
      500: "Something went wrong while loading the attachment",
    },
    { enabled: !!issue?.media },
  );

  useDocumentTitle(
    issue?.subject ? `Report - ${issue.subject}` : "Report Details",
  );

  const { role, username, email, submittedAt } = issue?.submittedBy || {};
  ...
  const handleUpdateStatus = async () => {
    ...
    const response = await updateIssue(reportId, status);

    if (response.success) {
      addToast(response.message || "Status updated successfully", "success");
  ...
  return (
    <div className="px-6 py-5 space-y-6 text-main">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          <div className="flex flex-wrap items-baseline gap-2">
            <h1 className="text-h3 font-semibold">Report by {username}</h1>

            <span className="text-emphasis text-muted">({email})</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-16 gap-y-6">
        {/* Subject */}
        <div className="space-y-1">
          <p className="text-h5 font-medium">Subject</p>

          <p className="text-body text-main">{issue?.subject}</p>
        </div>

        {/* Category */}
        <div className="space-y-1">
          <p className="text-h5 font-medium">Category</p>
      ...
      <div className="space-y-2">
        <p className="text-h5 font-medium">Description</p>

        <p className="text-body text-main leading-7">{issue?.description}</p>
      </div>
      ...
      {issue?.media && (
        <div className="space-y-2">
          <div className="flex items-center gap-1">
            <p className="text-h5 font-medium">Attachment</p>
          </div>
